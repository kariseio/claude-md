...
- 재진술 게이트를 2단계로 나눔. 요청 자체가 여러 갈래로 읽히면 6칸 형식 대신 선택지(2~4개, 각 한 줄, 추천 하나 표시)를 먼저 내고 멈춘다. 갈래가 정해진 뒤에만 재진술.
- 재진술에서 채울 수 없는 줄은 생략. "확정 후 제시" 같은 자리채움 문장 금지.
- 출력 라벨을 한글 네 글자 명사구로 통일: 요청 해석 / 적용 범위 / 변경 파일 / 변경 제외 / 완료 기준 / 확인 필요. 규칙 본문은 영어 유지.
- 1번 섹션 도입부를 한 문장으로 축약 (늘어난 줄 수 상쇄).

### 근거

- KMS credential 정리 요청에 대한 재진술 출력: 6칸 중 3칸이 "확정 후 제시", 실제 결정 사항인 선택지 4개는 맨 아래, 각 선택지가 2~3절, 추천 없음. 읽는 부담이 오독 방지 효과를 상쇄.
- 영어 라벨과 한국어 내용이 여섯 줄마다 번갈아 나와 읽기 어려움. "건드림", "모호"는 명사가 아니라 라벨로 읽히지 않음.
